...
namespace ControlCantina
{
    public partial class Barra : UserControl
    {
        ...
        private void ServirCopa(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(string.Format("NO QUEDA MAS LIQUIDO"), "Cantinero", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                String cadenaBotella = botella;
                MessageBox.Show(string.Format("SIRVIENDO! {1}ml\n{0}", cadenaBotella, cantidadServida), "Cantinero", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
